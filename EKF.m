...
if isempty(init_flag)
    ...
    R_psi(eta(3)) * nu
   -inv(data.T_b)*b
   -inv(data.M)*data.D*nu + inv(data.M)*R_psi(eta(3)).'*b];
   
    f_x_jacobi_sym = jacobian(f_x(xi, eta, b, nu),...
                            [xi.' eta_sym.' b.' nu.']);
...
K = P_pri * data.H' * inv(data.H * P_pri * data.H' + data.R);
...
Psi = eye(size(P_pri)) + data.Ts * f_x_jacobi(x_hat(:));
Gamma = data.Ts * data.E;

P_pri = Psi * P * Psi' + Gamma * data.Q * Gamma';
x_pri = f_xuk;
